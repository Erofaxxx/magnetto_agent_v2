...
| **Динамический хвост** | `DynamicContextMiddleware._dynamic_block()` | **каждый вызов** (сегодня + НДС) |

Динамический блок идёт **после** `cache_control`-границы → не попадает в Anthropic cache.
...
В `create_deep_agent(middleware=[...])` — порядок в списке:

| № | Middleware | Когда срабатывает | Что делает |
|---|---|---|---|
| 1 | `CachingMiddleware` | `wrap_model_call` (outermost) | ставит `cache_control: ephemeral` на последний блок system + последний ToolMessage + последний HumanMessage |
| 2 | `BudgetMiddleware` | `wrap_model_call` | считает tool-calls в turn; по превышении лимита подмешивает «budget notice» в system |
| 3 | `RoutingEnforcer` | `wrap_tool_call` | перехватывает сложные `clickhouse_query` без предшествующей делегации — возвращает блокирующий `ToolMessage` |
| 4 | `HardcodeDetector` | `wrap_tool_call` | ловит `pd.DataFrame({...: [литералы]})` в `python_analysis` |
| 5 | `DynamicContextMiddleware` | `wrap_model_call` (innermost) | дописывает «Сегодня: YYYY-MM-DD / НДС в РФ: 22%» **после** cache_control |
...
После `load_subagents()` в `agent_factory.py` каждому spec'у добавляются:
- `CachingMiddleware` — cache_control на стабильную часть system prompt
- `DynamicContextMiddleware` — сегодня+НДС в конце, вне кэша
...
```
system:                                                  ← весь блок кэшируется
  <body SUBAGENT.md с подставленным {schema_section}>
  <Индекс доступных skills: имена + descriptions из frontmatter>

  <<< cache_control: ephemeral >>>

  Сегодня: YYYY-MM-DD. ... (DynamicContextMiddleware)     ← вне кэша

messages:
  HumanMessage(description)                              ← то, что main передал в task
  ... (далее цикл tool_call → tool_result → ai → ...)
```

Исторические сообщения основной сессии subagent **не видит** — он всегда стартует с чистой истории на свой вопрос.
...
1. **System prompt** — на последний блок (AGENTS.md + data_map + skills index + SUBAGENT.md body — все кэшируются).
2. **Последний ToolMessage** в messages (граница «история до текущего turn»).
3. **Последний HumanMessage** (граница «новый пользовательский ввод»).

### 6.1 Что НЕ в кэше

- Динамический блок «Сегодня / НДС» (`DynamicContextMiddleware` добавляет ПОСЛЕ cache_control)
- Свежий пользовательский ввод (он и есть тот самый последний HumanMessage)
- Любые tool_call / tool_result **после** точек cache_control
