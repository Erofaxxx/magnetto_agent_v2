...
|---|---|---|
| `write_todos(...)` | план на 2+ шага **до** первого делегирования | обязателен для multi-step |
| `think_tool(thought)` | фиксация гипотезы/плана | — |
...
| `python_analysis(code, parquet_path)` | **единственный** post-processor parquet от subagent (графики, merge, производные) | `HardcodeDetector` блокирует `pd.DataFrame({...})` с литералами |
| `list_tables()` | резерв «есть ли такая витрина», для роутинга — не для SQL | — |
| виртуальная ФС (`ls`, `read_file`, `write_file`, `glob`, `grep`) | работа с `/plots/`, `/memories/`, подгрузка `/skills/<x>/SKILL.md` | — |

**`clickhouse_query` у main НЕТ** (апрель 2026). Архитектурное разделение: main — роутер + post-processor, все SQL идут через `task(...)` / `delegate_to_generalist(...)`. Раньше `RoutingEnforcer` блокировал сложные SQL от main — теперь блокировать нечего, enforcer убран. `HardcodeDetector` остался на `python_analysis`.
...
| 2 | `CachingMiddleware` | `wrap_model_call` | ставит `cache_control: ephemeral` на последний блок system (включая только что добавленный today+VAT) + на последний ToolMessage (граница истории) + на последний HumanMessage (свежий ввод). Всё ДО этих точек уходит в Anthropic ephemeral-кэш |
| 3 | `BudgetMiddleware` | `wrap_model_call` | считает tool-calls в turn; по превышении лимита подмешивает «budget notice» в system |
| 4 | `HardcodeDetector` | `wrap_tool_call` | ловит `pd.DataFrame({...: [литералы]})` в `python_analysis` |
...
Набор собирается фабрикой `_make_subagent_tools(schema_tables)` в `agent_factory.py` — каждому subagent'у свой:

```python
def _make_subagent_tools(schema_tables):
    return [
        clickhouse_query,
        python_analysis,
        think_tool,
        make_sample_table_tool(allowed_tables=schema_tables),   # scoped на свои таблицы
    ]
```

- `clickhouse_query` — основной способ работы с БД.
- `python_analysis` — для построения графиков/DataFrame из parquet.
- `think_tool` — рефлексия/план.
- `sample_table(table_name, n=5)` — **живой срез**: первые N строк из таблицы (только из `schema_tables` этого subagent'а, иначе отказ). Используется чтобы узнать реальные значения enum-колонок, форматы Array-полей, NULL-паттерны — до того как писать сложный SQL. Автофильтр по последнему `report_date`/`snapshot_date` или `WHERE date < today()`. Результат cap в ~4KB, длинные строки truncate до 200 символов.

**НЕ имеют** (в отличие от main):
- `list_tables`, `delegate_to_generalist`, `task`, `write_todos`, виртуальная ФС (`ls/read_file/...`).

Это значит: subagent не может вложенно делегировать дальше, не строит todos, не читает `/memories/`. Его работа — один изолированный вопрос + 1-3 SQL (c sample_table — часто 1) + финальный текст.

**Почему `describe_table` не нужен:** полная схема таблиц из `schema_tables:` уже в system prompt через `{schema_section}`, подставляется при старте в `subagent_loader`. Дополнительного tool'а чтобы «узнать схему» не нужно.
...
- Tools generalist получает через ту же фабрику `_make_subagent_tools(tables)` — т.е. включают `sample_table` со scope = переданные `tables`. Значит generalist тоже может подсмотреть живые данные.
