...
- **Не пиши прозу между tool-calls.** Между шагами рассуждай в `think_tool` (1–3 короткие пункта). Не пиши черновик отчёта в обычном assistant-сообщении — это попадает в кэш-префикс на каждой следующей итерации и быстро раздувает контекст. Финал пиши **один раз**, сразу в `SubagentResult`.
