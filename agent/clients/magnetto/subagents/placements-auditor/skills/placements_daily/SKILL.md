...
- **`python_analysis` — максимум один раз, и НЕ для печати таблиц.** Размеченный датасет уже в parquet. В `python_analysis` печатай в stdout только короткие counts/sums (1–5 строк, типа «excludes per campaign: 705590486=46, 701940874=18, …»). **Не делай `print(df.head(30))`, `print(df.to_string())`, `result = df.to_markdown()`** — это копирует таблицы в контекст по 8K токенов на каждый вызов. Если нужен пост-агрегированный срез (по кампаниям, по категориям площадок, что угодно) — сохраняй его в **новый parquet** через `df_new.to_parquet('/parquet/<имя>.parquet')` и кладёшь путь в `parquet_paths` финального ответа. UI его покажет таблицей с пагинацией. Один parquet видит всё, дублировать в md не нужно.
...
(ровно топ-5 строк на кампанию в md; sort by cost DESC; **ВСЕ 11 колонок целей обязательны**. Если в кампании 1–4 исключений — выводи столько, сколько есть. Полный размеченный список — в parquet, см. Раздел 10. После таблицы добавь строку: «Полный список — N площадок на исключение, открой parquet `<имя>.parquet`».)
...
4. ✅ Каждая кампания с исключениями имеет: заголовок (имя+ID), микро-сводку, **отдельную** таблицу с ровно топ-5 строк и ВСЕМИ 11 колонками целей, мини-секции «Что работает» и «Что произойдёт после»? Полный список всех исключений — в `parquet_paths`?
