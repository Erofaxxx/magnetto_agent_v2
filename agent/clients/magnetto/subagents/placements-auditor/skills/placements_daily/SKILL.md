...
**Реакция на budget-уведомления (от системы).** Подагент имеет жёсткий лимит итераций. Тебе придёт **только два** уровня уведомлений в самом конце: `🚨 Почти исчерпан` (осталось 1–2 итерации) и `⛔ ЛИМИТ ИСЧЕРПАН` (0). Soft-уведомлений раньше нет — экономим кэш. Реагируй: на `🚨` — сразу собирай `SubagentResult` из того, что уже есть, без новых tool-вызовов; на `⛔` — финал **без инструментов**, в `warnings` отметь «лимит итераций достигнут, что осталось недосчитанным: X». Лучше частичный осмысленный отчёт, чем пустой.
