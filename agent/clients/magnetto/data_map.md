...
- **`placements_daily`** — 1 строка = (`Date` × `cabinet_name` × `CampaignId` × `Placement`). Только РСЯ (`AdNetworkType='AD_NETWORK'`), пустые `Placement` отсеяны. Поля: `cost`, `clicks`, `impressions`, `bounces`, `purchase_revenue`, **18 goal_* колонок** (`goal_all_leads` ← главный референс лидов 314553735; CRM: `goal_crm_paid`/`goal_crm_created`/`goal_crm_rejected`; звонки: `goal_call`/`goal_call_unique`/`goal_call_unique_target`/`goal_call_target`; phone-clicks: `goal_phone_click_mcc`/`goal_phone_click_mag`; `goal_auto_form`, `goal_trash_traffic`). Партиционирование `toYYYYMM(Date)`, ORDER BY `(cabinet_name, Date, CampaignId, Placement)`. ⚠ **`cabinet_name` обязателен в WHERE** — иначе смешает 4 проекта. ⚠ **PascalCase** для `Date`/`CampaignId`/`CampaignName`/`Placement`. Используется только новой методикой аудита РСЯ (триггер `АУДИТ_РСЯ_V2`); старый snapshot-аудит живёт в `bad_placements` у `direct-optimizer`. **Направляй в subagent `placements-auditor` через `task(...)`.**
- **`placements_goal_calibration`** — 1 строка = (`cabinet_name` × `goal_id`) за rolling-90д. Ровно 32 строки (4 кабинета × 8 целей: 4 типа звонков + 2 phone_click + auto_form + trash). Поля: `goal_name` (имя как в `placements_daily`), `period_from`, `period_to`, `events`, `placements_with_goal`, `clicks_at_those_placements`, `events_per_1000_clicks`, **`leads_per_event`** (главный «вес» — 1 событие цели = X лидов `goal_all_leads`), `leads_per_click_at_active`, `base_leads_per_click_in_cabinet`, `quality_lift`, `corr_with_all_leads`, `n_placements_for_corr`, **`confidence`** (`high`/`medium`/`low`). ⚠ **При `confidence='low'` вес НЕ использовать** — нерепрезентативно. CRM-цели и `goal_all_leads` тут НЕ калибруются (служат референсами). Партнёр к `placements_daily` для `lead_equivalent`. Направляй в subagent `placements-auditor`.
