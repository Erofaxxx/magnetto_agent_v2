...
Параллельные массивы `priority_goal_ids` и `priority_goal_values`. Разворачивай через `arrayJoin`:
...
## Когда НЕ останавливаться в command_center

- Вопрос про **конкретные ключевые слова**, минус-слова, bid_zone → делегируй в `direct-optimizer` (bad_keywords/bad_placements/bad_queries + campaigns_settings).
- Вопрос про **ключевую фразу, которая ворует бюджет** → delegate direct-optimizer.
...
Не раскручивай drill, если юзер не просил причин.
